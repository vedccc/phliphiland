...
import { RefreshCw, Trash2, X, Plus, HelpCircle, Link, Copy, ArrowRight } from "lucide-react";
...
export default function PropertiesKB() {
  ...
  const deleteProperty = async (p: Property) => {
    if (!confirm(`Delete "${p.name}" and all of its data (KB, tickets, requests)? This cannot be undone.`)) return;
          ...
          {properties.map((p) => {
            ...
            return (
              <button
                ...
                onContextMenu={(e) => { e.preventDefault(); if (isSuperAdmin) deleteProperty(p); }}
                title={isSuperAdmin ? "Right-click to delete this property" : undefined}

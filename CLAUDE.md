...
**Phillip Island Host — AI Guest Agent.** An automated short-term-rental guest messaging system for Phillip Island Host (Property Management). An AI agent receives guest messages from booking platforms (Airbnb, Booking.com, email, etc.) via **Guesty**, classifies them, and either answers from a knowledge base, creates maintenance tickets, processes extra requests (host-approved by SMS link), or escalates to a human.

This is a Guesty-based clone of the prior Hospitable "Sonic AI / Iris de Mer" project. Design + plan: `docs/superpowers/specs/2026-06-22-phillip-island-guesty-clone-design.md` and `docs/superpowers/plans/2026-06-22-phillip-island-guesty-clone.md`.

## Tech Stack

- **Orchestration:** Trigger.dev (project `proj_jroitttjzbwjhdzzisjv`)
- **Webhook ingress:** Supabase Edge Function `guesty-webhook` — receives Guesty `reservation.messageReceived`, normalizes guest messages, forwards to Trigger.dev.
- **AI:** Claude Sonnet 4.6 (`claude-sonnet-4-6`) via Anthropic API — 3 distinct AI calls (Coordinator, KB Answerer, Urgency Assessor)
- **Database:** Supabase (Postgres + Auth + RLS) — project `ictumlksmzjenevtaqvp`
- **External APIs:** Guesty Open API (`open-api.guesty.com/v1`, OAuth2 client-credentials), SMSAPI
- **Hosting:** Render (dashboard frontend)

## Guesty integration (PMS)

- Auth: OAuth2 client-credentials → bearer token, cached ≤24h (Guesty caps 5 tokens/24h). See `backend/trigger/helpers/guesty.ts`.
- **Messaging is keyed by `conversationId`, not reservation id.** The inbound webhook supplies both; the agent threads `conversationId` + the channel `module` through the run and replies on the same channel via `POST /communication/conversations/{id}/send-message`.
- Properties = Guesty **listings** (`properties.guesty_listing_id`). Reservations are referenced by Guesty `_id` (stored as `reservation_uuid` text on tickets/extras/checklists).

## Architecture

The system uses a **Coordinator → Sub-Workflow** pattern inside a single Trigger.dev workflow (`backend/trigger/messaging/main-agent.ts`):

1. **Coordinator (AI Step #1)** — An agentic loop with tool-use. Classifies the guest message and calls one of five tools. Duplicate tool calls with identical input are de-duplicated within a turn.
2. Each tool maps to a **hard-coded sub-workflow**:
   - **A** (`use_knowledge_base`) — Loads KB from Supabase, calls KB Answerer AI (#2). If no answer → escalate (hard stop).
   - **B** (`raise_maintenance_ticket`) — Calls Urgency Assessor AI (#3), creates ticket, sends SMS alerts.
   - **C** (`process_extra_request`) — Checks `allowed_extras`; if allowed, records pending + sends host an SMS approval link; host decision re-fires the agent (carrying `guesty_conversation_id`/`guesty_channel_module`) to reply to the guest.
   - **E** (`handle_checkin_checkout`) — Notifies the cleaning team by SMS.
   - **D** (`escalate_to_human`) — **HARD STOP.** Logs KB gap, sets 8hr cooldown, sends SMS. No reply to guest.

The agent composes and sends the final guest reply via Guesty. Exception: Sub-Workflow D terminates without replying.

## Critical Rules

- **Sub-Workflow D = no reply to guest.** Never guess/placeholder when escalating. Silence.
- **Cooldowns are per-property**, not per-guest. When active, ALL messages for that property are ignored.
- **Guest messages only** — the `guesty-webhook` ingress drops non-`fromGuest` messages; the `ALLOWED_RESERVATION_UUIDS` env gates which reservations the agent replies to (unset = safe/off, `*` = all).
- **No reservations feature** — this client does not sync/list reservations (the dashboard Reservations page, import-reservations, reservation-webhook, and list-reservations were removed). The agent still uses Guesty reservation IDs internally to identify the guest and reply.

## Database Tables (Supabase)

`profiles`, `properties`, `knowledge_bases`, `kb_gap_log`, `cooldowns`, `maintenance_tickets`, `allowed_extras`, `extra_requests`, `extras_approval_tokens`, `sms_recipients`, `urgency_categories`, `agent_activity_log`, checklist tables (`checklist_templates`, `checklist_template_items`, `checklist_instances`, `checklist_instance_items`), and an (unused, retained) `reservations` table read by the public checklist/extras pages for guest names. RLS enabled (authenticated = full access, anon = blocked). Full schema: `backend/supabase/schema.sql`.

## Trigger.dev Workflows

1. **Main Agent Workflow** — triggered by `guesty-webhook`. Processes guest messages through the AI agent loop.
2. **Property Sync Workflow** — pulls **listings** from Guesty, upserts into `properties`, then generates KB Q&A via Claude.
3. **Supabase Keep-Alive** — daily scheduled trigger.
...
SUPABASE_ACCESS_TOKEN     # personal access token, used by the Supabase MCP launcher
ANTHROPIC_API_KEY
TRIGGER_SECRET_KEY
PUBLIC_LINK_SIGNING_SECRET
DASHBOARD_HOST
PHILLIP_ISLAND_PHONE      # emergency phone shown in the AI message footer
```
